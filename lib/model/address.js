...
Address._deriveAddress = function(scriptType, publicKeyRing, path, m, coin, network) {
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      bitcoreAddress = BCHAddressTranslator.translate(Bitcore[coin].Address.createMultisig(publicKeys, m, network), 'legacy', 'copay');
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      bitcoreAddress = BCHAddressTranslator.translate(Bitcore[coin].Address.fromPublicKey(publicKeys[0], network), 'legacy', 'copay');
      break;
  }
